...
目录内运行时、源码工作区检查、显式构建助手和 Shell 解析适配已经实现，并在全新官方 DSH checkout 上通过便携启动 smoke。

## 目录约定

桌面壳安装在目标 DSH 根目录的 `desktop-shell` 子目录：
...
打包后的壳默认把自身目录的上一级作为 DSH 根目录。`DSH_DESKTOP_RUNTIME_DIR` 可为开发和诊断显式指定另一目录；未设置时不搜索磁盘、不连接其他 DSH 实例，也不回退到壳内副本。源码运行默认使用 Shell 自身固定的开发依赖。

## 支持的 DSH 布局

### 已安装运行时

根目录包含 `package.json` 与 `node_modules/@deepseek-ai/dsh/package.json`。壳从该包清单建立模块解析器，并验证 app-boot、cmdline、launch-environment 和 host-webserver 的公开入口。

### 官方源码工作区

根目录包含 `package.json`、`pnpm-lock.yaml`、`pnpm-workspace.yaml` 与 `apps/cli/package.json`。源码工作区的发布包不位于根 `node_modules`；壳以 `apps/cli/package.json` 为安装锚点，通过 `apps/cli/node_modules` 解析工作区包，并要求相关 `lib` 入口已经生成。
...
不满足任一条件时快速失败。壳不自动选择其他版本，也不保留 `resources/dsh-runtime` 静默回退。
...
4. 重新执行运行时就绪检查。
5. 成功后重启桌面壳；失败时保留完整命令输出，不回退其他运行时。

构建助手不编辑 DSH 源码、配置或插件。依赖安装会更新 `node_modules`，构建会生成或更新 `lib` 等产物；恢复页必须在执行前明确提示这两类写入。

Electron 的 Node 私有 ESM Loader 在 Cordis 内不可用时，Shell 注册一个进程内同步解析钩子：先执行 Node 默认解析，只有裸包名失败时才改用 Profile 的 `profiles/node_modules` 回退目录。钩子随 Host 退出撤销，不写入 DSH 目录，也不改变插件内容。
...
- 运行数据仍使用桌面壳隔离目录，不自动接入正式 `DSH_HOME`。
