...
`0.1.3` 安装器大小为 `100,065,398` 字节，SHA-256 为 `FC18685290BA229628CD116958A025610EA27C5C58C2D38829057446F3E6E65C`。`latest.yml`、安装器和 blockmap 的版本与文件名一致。`0.1.0` 的历史安装器因未携带自动更新器依赖不能自行升级；`0.1.1` 到 `0.1.2` 的真实在线更新已通过。标题栏进度代码从 `0.1.3` 开始生效，因此必须以 `0.1.3` 为旧客户端、发布更高版本后才能完成真实进度显示回归。
