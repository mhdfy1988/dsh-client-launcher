...
用户可见产品名称为“DSH 客户端启动器”，仓库和 package 名为 `dsh-client-launcher`。历史 `DSH_DESKTOP_*` 环境变量、Cordis 启动标识和更新缓存名保持兼容；这些标识不会改变运行时兼容范围。

| 项目 | 固定版本或范围 | 当前结论 |
| --- | --- | --- |
| Windows | Windows 11 x64 | `0.1.3` 已验证目标；Windows 10 未验证 |
...
| Profile | `web` | 源码/smoke 位于隔离 `.poc/dsh-home`；正式包使用 Harness 解析出的数据目录 |
