...
- 当前 POC 只保留已经验证的托盘、窗口状态、自绘标题栏、主题同步和受配置控制的自动更新协调器；正式签名、真实在线升级、失败回滚和插件市场仍必须单独验证。
- 正式便携包和安装器必须携带自动更新器的生产依赖闭包，并用真实 Electron 从最终 `app.asar` 加载 `electron-updater.autoUpdater`；源码测试或显式关闭更新的 smoke 不能替代该门禁。
